...
/**
 * Start before testing: firebase emulators:start --project=firestore-proxy
 * 
 * TODO: nullable tests
...
async function createDocumentAndLoadedFirestoreProxy(data: Record<string, unknown>) {
  const docRef = await addDoc(testCollection, data);
  const fp = new FirestoreProxy(testDataSchema, docRef);
  await fp.load();
  return fp;
}
...
describe("load", () => {
  it("must be called before referencing pad.", async () => {
    const fp = new FirestoreProxy(testDataSchema, doc(testCollection, "foo"));
  ...
  it("throws for a created but not yet written doc.", async () => {
    const fp = FirestoreProxy.createDoc(testDataSchema, testCollection, {
  ...
  it("always reads from Firestore if force is set.", async () => {
    const fp = await createDocumentAndLoadedFirestoreProxy({
      email: "bob@example.com",
    });
...
describe("record of primitives", () => {
  ...
  it("reads an entry.", async () => {
    const fp = await createDocumentAndLoadedFirestoreProxy(initialState);
...
describe("createDoc", () => {
  ...
  it("does not provide a doc ref until write is called.", async () => {
    const fp = FirestoreProxy.createDoc(testDataSchema, testCollection, initialState);
  ...
  it("adds a document with a given ID.", async () => {
    ...
    const fp = FirestoreProxy.createDoc(
      testDataSchema,
...
describe("copy", () => {
  ...
  it("performs a deep copy of a document.", async () => {
    const fp1 = FirestoreProxy.createDoc(testDataSchema, testCollection, initialState);
    const fp2 = fp1.copy();
  ...
  it("can copy into a specified ID.", async () => {
    const fp1 = FirestoreProxy.createDoc(
      testDataSchema,
